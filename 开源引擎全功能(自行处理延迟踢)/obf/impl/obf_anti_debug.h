...
// ════════════════════════════════════════════════════════════
// ⚠️ 2026-09-02 库 bug 修复 (VS C2065 "ObfNonBlockingCodeStaticData 未声明"):
//   obf_literal.h::ObfLiteralContext_version<5>::final_surjection 无条件
//   引用 ObfNonBlockingCodeStaticData<void>::zero_if_not_being_debugged(),
//   但简化分支 (本条 #else) 只定义了 ObfNaiveSystemSpecific/ObfNonBlockingCode,
//   缺 ObfNonBlockingCodeStaticData → 定义了 ITHARE_OBF_NO_ANTI_DEBUG 或
//   无 ITHARE_OBF_SEED 的 TU 编译必崩。补空桩: zero_if_not_being_debugged
//   恒 0 → 字面量仅做常量冲销, 无反调试干扰 (禁用库内置反调试 = 预期
//   行为; 我们的 反调试.h 才是真正的反调试层)。
// ════════════════════════════════════════════════════════════
template<class Dummy>
struct ObfNonBlockingCodeStaticData {
	template<ITHARE_OBF_SEEDTPARAM seed2>
	ITHARE_OBF_FORCEINLINE static uint64_t zero_if_not_being_debugged() { return 0; }
};
