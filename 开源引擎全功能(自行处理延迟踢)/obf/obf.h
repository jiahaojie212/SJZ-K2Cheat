...
// ⚠️ 2026-09-02 工程集成修复 (VS C2589/C2760):
//   Windows.h 的 min/max 宏会把库内 std::numeric_limits<T>::min()/max()
//   模板代码展开成错误语法 (obf_common.h 324-329 行实锤)。
//   用 push_macro/pop_macro 在库编译窗口内临时摘下、结束后恢复 ——
//   工程其他代码仍依赖 min/max 宏 (如 引擎.cpp 的 min(宽,高)),
//   因此不能全局 NOMINMAX, 只能库内隔离。
#ifdef min
#pragma push_macro("min")
#pragma push_macro("max")
#undef min
#undef max
#define ITHARE_OBF_RESTORE_MINMAX
#endif
...
#ifdef ITHARE_OBF_RESTORE_MINMAX
#pragma pop_macro("min")
#pragma pop_macro("max")
#undef ITHARE_OBF_RESTORE_MINMAX
#endif
